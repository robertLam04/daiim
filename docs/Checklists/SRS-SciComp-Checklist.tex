...
\item Follows the template, all parts present
  \begin{todolist}
  \item Selected template is appropriate for the project
...
\item Reference Material
  ...
  \item All non-SI units should be checked to see if they are really what is
  ...
  \item The units balance for all equations
  ...
  \item All symbols have unique definitions; that is, no two symbols have the
  same definition
...
\item Introduction
  ...
  \item There is a reference in the Organization of the Document section to the
...
\item Solution Characteristics Specification
  ...
  \item When the assumptions are listed, traceability is given to where they are used
  ...
  \item All GDs explicitly refine at least one TM.
  \item All GDs invoke (use) at least one assumption.
  ...
  \item All symbols used in every equation are defined in the Description
  \end{todolist}
