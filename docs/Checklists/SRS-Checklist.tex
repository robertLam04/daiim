...
\item Follows writing checklist (full checklist provided in a separate document)
  ...
  \item Writing style is appropriate
  \item Any points made by your TA on the previous documents that should also
  apply to this document, have been addressed
  \end{todolist}
...
\item Follows the template, all parts present
  ...
  \item Symbolic names are used for quantities, rather than literal values (You
  shouldn't say 3-second response time, you should use a symbolic constant. If
  you do say a 3-second response time, a rationale for this value should be
  provided, or possibly you should be focusing on the real requirement, rather
  than this derived requirement.)
  \end{todolist}
...
\item Reference Material
  ...
  \item Hyperlinks and/or bibliographic references to external resources
  \end{todolist}
...
\item Nonfunctional Requirements
  \begin{todolist}
  \item NFRs are verifiable (use fit criteria as appropriate)
  \item NFRs are unambiguous
...
\item Requirements
  \begin{todolist}
  \item Rationale is required for non-obvious requirements
...
\item Avenue Rubric
  ...
  \item You need over 5 peer review issues for both the SRS and HA for full
  marks.
  \item Specific timeline for phase in plan?
  \item Likely/unlikely changes?
  \item Traceability?
  \item Part of the SRS document has been formalized
  \end{todolist}

\end{itemize}

If you are using the Scientific Computing SRS template, a
\href{https://github.com/smiths/capTemplate/blob/main/docs/Checklists/SRS-SciComp-Checklist.pdf}
