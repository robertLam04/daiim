...
The following checklist should be applied to all documents that you write.
...
\item \LaTeX{} points
  ...
  \item Long names in math mode use either mathit or text, or equivalent:
    ...
    accordingly.)
  \item The \texttt{Comments.text} file is provided for ease of adding comments
  to a LaTeX file.
  ...
  \item If using \texttt{natbib} then \texttt{citet} and \texttt{citep} are used
  appropriately
...
\item Spelling, Grammar and attention to detail
  ...
  \item Figures use a vector format over a bitmap format when possible
  \end{todolist}
